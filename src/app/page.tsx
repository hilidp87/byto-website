...
export default async function HomePage() {
  ...
  let outfitSources: Record<string, { topSrc: string; bottomSrc: string }> = {};

  try {
    const [fetchedOutfits, fetchedProducts, fetchedPositions] = await Promise.all([
      ...
      prisma.outfitPosition.findMany(),
    ]);
    ...
    for (const pos of fetchedPositions) {
      if (pos.topSrc && pos.bottomSrc) {
        outfitSources[pos.outfitId] = { topSrc: pos.topSrc, bottomSrc: pos.bottomSrc };
      }
    }
  } catch {
  ...
  return (
    <>
      <HeroSection />
      <OutfitScrollShowcase sources={outfitSources} />
      <OutfitCategories />
